...
export function Header({
  ...
  const primaryNavItems = [
    { id: 'pipeline', label: 'Architecture Map', icon: Activity },
    { id: 'registry', label: 'Content Registry', icon: Database },
    { id: 'quarantine', label: 'Quarantine Queue', icon: Shield },
    { id: 'chatgpt', label: 'ChatGPT Retrieval', icon: Sparkles },
    { id: 'specs', label: 'Developer Specs', icon: Sliders },
  ];
        ...
        <nav className="hidden lg:flex items-center gap-0.5 bg-[var(--color-panel-subtle)] p-1 rounded-lg border border-[var(--color-border)]">
          {primaryNavItems.map((item) => {
            ...
            return (
              ...
              >
                <Icon className={`h-3.5 w-3.5 shrink-0 ${isActive ? 'text-amber-600 dark:text-amber-400' : ''}`} />
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-60 bg-[var(--color-surface)] border border-[var(--color-border)] p-1.5">
              ...
              >
                ...
                <div>
                  <div className="font-bold text-[var(--color-text-primary)]">Boundary & Chaos Test</div>
                  <div className="text-xs text-[var(--color-text-muted)] mt-0.5">Test API outages & client leak defenses</div>
                </div>
              </DropdownMenuItem>
